use crate::zk::r1cs::{R1CSMatrix, SpartanInstance, solve_witness_from_r1cs};
...
pub fn remove_aliases(instance: &SpartanInstance) -> SpartanInstance {
    ...
    for i in 0..num_circuit_inputs {
        col_remap.insert(num_vars + 1 + num_circuit_outputs + i,
...
pub fn remove_scales(instance: &SpartanInstance) -> SpartanInstance {
    ...
    if removed_count > 0 {
        eprintln!("[remove_scales] Skipped removing {} scale rows (output still in use)", removed_count);
    }
...
mod tests {
    ...
    fn test_remove_aliases_stress() {
        ...
        for i in 0..num_iterations {
            ...
            let ch = Challenge::generate_instance(&seed, &Difficulty { delta: 1 }).unwrap();
    ...
    fn test_debug_iteration_14() {
        let seed = [0u8, 0u8, 0u8, 14u8, 0u8, 0u8, 0u8, 0u8,
                    0u8, 0u8, 0u8, 0u8, 0u8, 0u8, 0u8, 0u8,
                    0u8, 0u8, 0u8, 0u8, 0u8, 0u8, 0u8, 0u8,
                    0u8, 0u8, 0u8, 0u8, 0u8, 0u8, 0u8, 0u8];

        let ch = Challenge::generate_instance(&seed, &Difficulty { delta: 1 }).unwrap();
        eprintln!("[debug] C0: {} cons, {} vars, {} pub_io, {} outputs",
            ch.circuit_c0.num_cons, ch.circuit_c0.num_vars, ch.circuit_c0.num_inputs, ch.circuit_c0.num_outputs);

        let result = remove_aliases(&ch.circuit_c0);
        eprintln!("[debug] After remove_aliases: {} cons, {} vars, {} outputs",
    ...
    fn test_verify_alias_detection() {
        ...
        for &(row, col, bytes) in &c0.A { a_rows[row].push((col, bytes)); }
        for &(row, col, bytes) in &c0.B { b_rows[row].push((col, bytes)); }
        for &(row, col, bytes) in &c0.C { c_rows[row].push((col, bytes)); }
        ...
        eprintln!("[verify] Total: {}, num_vars={}, num_inputs={}",
            private_to_private + private_to_public + public_to_public + mixed,
            num_vars, c0.num_inputs);
        eprintln!("[verify] Public I/O range: {}..{} (const), {}..{} (outputs), {}..{} (inputs)",
            num_vars, num_vars+1,
            num_vars+1, num_vars+1+ch.num_circuit_outputs,
            num_vars+1+ch.num_circuit_outputs, num_vars+1+c0.num_inputs);
    }
    ...
    fn test_remove_aliases_and_scales_stress() {
        ...
        for i in 0..num_iterations {
            ...
            match solve_witness_forward(&result, ch.num_circuit_outputs, &x_eval) {
                Ok(_) => {}
                Err(e) => {
                    eprintln!("[ITERATION {}] FAIL: seed={:02x}{:02x}{:02x}{:02x}... error={:?}",
                        i, seed[0], seed[1], seed[2], seed[3], e
                    );
                    failures.push((i, seed, format!("{:?}", e)));
                }
    ...
    fn test_full_challenge_pipeline() {
        ...
        for i in 0..num_iterations {
            ...
            for (name, optimizer) in [
                ("remove_aliases", remove_aliases as fn(&SpartanInstance) -> SpartanInstance),
                ("remove_aliases_and_scales", remove_aliases_and_scales),
            ] {
                match solve_challenge(&ch, optimizer) {
                    Ok(_) => {}
                    Err(e) => {
        ...
        assert_eq!(failures, 0, "Full pipeline had {} failures out of {} iterations", failures, num_iterations);
        eprintln!("[pipeline] All {} iterations passed (both algorithms)", num_iterations);
    }
    ...
    fn test_optimization_quality_distribution() {
        ...
        let min_aliases = epsilons_aliases.iter().fold(f64::INFINITY, |a, &b| a.min(b));
        let max_aliases = epsilons_aliases.iter().fold(f64::NEG_INFINITY, |a, &b| a.max(b));

        eprintln!("[quality] Sampled {} challenges:", num_samples);
        eprintln!("[quality]   remove_aliases: ε ∈ [{:.3}, {:.3}], mean={:.3}",
            min_aliases, max_aliases, mean_aliases);
        eprintln!("[quality]   remove_aliases_and_scales: mean ε={:.3}", mean_both);

        // Sanity checks
        assert!(mean_aliases > 0.05, "Alias removal should reduce constraints by at least 5%");
        assert!(mean_both >= mean_aliases, "Combined should be at least as good as aliases alone");
    }
}
